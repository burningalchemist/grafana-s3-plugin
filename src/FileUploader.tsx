...
export const FileUploader: FC<Props> = (props) => {
    ...
    const response = await getBackendSrv().datasourceRequest({
      ...
      data: {
        queries: [
          {
            refId: 'A',
            query: props.profile ? 'UPLOAD PROFILE' : 'UPLOAD SESSION',
            orgId: props.orgId,
  ...
  const uploadFile = async (file: any, i: number, n: number) => {
    try {
      await authenticate();
      setProgress({
        busy: true,
      ...
      return await new AWS.S3.ManagedUpload({
        params: {
          Bucket: props.bucket,
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      {errors.map((err) => (
        <Box width="calc(100% - 200px)" key="label">
